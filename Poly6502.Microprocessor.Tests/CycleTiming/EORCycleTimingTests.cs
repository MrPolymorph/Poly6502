...
namespace Poly6502.Microprocessor.Tests.CycleTiming
{
    public class EORCycleTimingTests
    {
        ...
        private void SetupTruthTable()
        {
            ...
            {
                /* EOR */
                /* Immediate */   new CycleTruthData(0x49, 2),
                /* Zero Page */   new CycleTruthData(0x45, 3),
                /* Zero Page X */ new CycleTruthData(0x55, 4),
                /* Absolute */    new CycleTruthData(0x4D, 4),
                /* Absolute X */  new CycleTruthData(0x5D, 4, true),
                /* Absolute Y */  new CycleTruthData(0x59, 4, true),
                /* Indirect X */  new CycleTruthData(0x41, 6),
                /* Indirect Y */  new CycleTruthData(0x51, 5, true),
            };
        }
        ...
        [Test]
        public void Test_EOR_Cycle_Timing()
        {
            StringBuilder sb = new StringBuilder();

            foreach (var truth in _truthData)
            {
                TestEOR(truth.OpCode);
        ...
        public void TestEOR(byte opcode)
        {
            Operation op = _m6502.OpCodeLookupTable[opcode];

            Assert.IsTrue(op.OpCodeCompare(_m6502.EOR));
