...
namespace Poly6502.Microprocessor.Tests.CycleTiming
{
    public class ROLCycleTimingTests
    {
        ...
        private void SetupTruthTable()
        {
            ...
            {
                /* ROL */
                /* Accumulator */    new CycleTruthData(0x2A, 2),
                /* Zero Page */      new CycleTruthData(0x26, 5),
                /* Zero Page X */    new CycleTruthData(0x36, 6),
                /* Absolute */       new CycleTruthData(0x2E, 6),
                /* Absolute X */     new CycleTruthData(0x3E, 7),
            };
        }
        ...
        [Test]
        public void Test_ROL_Cycle_Timing()
        {
            StringBuilder sb = new StringBuilder();
            
            foreach (var truth in _truthData)
            {
                TestROL(truth.OpCode);
                ...
                {
                    AreEqual(truth.Cycles, takenCycles, $"opcode 0x{truth.OpCode:X2}", $"OpCode 0x{truth.OpCode:x2} Passed");
                    Console.WriteLine($"OpCode 0x{truth.OpCode:x2} Passed");
                }
                else
                {
                    if(takenCycles != truth.Cycles && takenCycles != truth.MaxPotentialCycles)
                        Fail($"Expected {truth.Cycles} or {truth.MaxPotentialCycles} cycles. Actual : {takenCycles}");
        ...
        public void TestROL(byte opcode)
        {
            Operation op = _m6502.OpCodeLookupTable[opcode];

            IsTrue(op.OpCodeCompare(_m6502.ROL));
